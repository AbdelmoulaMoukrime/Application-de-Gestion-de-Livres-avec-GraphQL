...
function displayBooks(books) {
    ...
    if (!books || books.length === 0) {
        booksList.innerHTML =
            '<div class="col-12"><p class="text-muted text-center">Aucun livre trouvé.</p></div>';
        return;
    }
                ...
                <div class="card book-card h-100">
                    ...
                    <div class="card-footer">
                        <button class="btn btn-sm btn-warning me-2"
                                onclick="editBook('${book.id}')">Modifier</button>
                        <button class="btn btn-sm btn-danger"
                                onclick="deleteBook('${book.id}')">Supprimer</button>
                    </div>
                </div>
            </div>
        `;
...
async function addNewBook() {
    ...
    const category = document.getElementById('category').value; // ex: "TECHNOLOGY"
